...
function SearchBarSmall() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post(
        ...
        navigate('/HotelList', { state: response.data })      
      });
    } catch (error) {
  ...
  return (
    <>
      <Card bordered={false} className="searchSmalllCard container">
        <Form
          layout="vertical"
          form={forms}
          initialValues={{
          ...
          <Card bordered={false} className="cardContainer">
            ...
            <Card.Grid style={gridStyle}>
              <Form.Item
                label="Date"
                name="tgl_masuk"
                rules={[{ required: true, message: "Check-in/out date is required" },]}
              >
